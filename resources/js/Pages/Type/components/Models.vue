            ...
            <div class="user-serchbox d-flex">
                ...
                <div class="searchbox ml-auto">
                    <nav class="navbar user-nav d-flex nav-mr-top">
                        ...
                        <a :href="route(link)" :active="route().current(link)" class="btn btn-primary btn-sm cls rg-top-btn"><i class="fa fa-plus"></i> Type</a>
                    </nav>
                                ...
                                <td v-if="$page.auth.user.user_type == 'staff'" class="text-right">
                                    <div class="dropdown dropdown-action ">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right " x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-91px, 27px, 0px);">
                                                <a class="dropdown-item" :class="$page.auth.user.id != row.created_by?'disabled':'' " :href="route('type.show', [row.id])"><i class="fa fa-pencil m-r-5"></i> View</a>
                                                <button  @click="deleteRow(row)"  :class="$page.auth.user.id != row.created_by?'disabled':'' "  class="dropdown-item"  data-toggle="modal" data-target="#delete_patient"><i class="fa fa-trash-o m-r-5"></i> Delete</button>
                                            </div>
                                        </div>
                                </td>
                                 <td v-else class="text-right">
                                        <div class="dropdown dropdown-action ">
                                            <a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="true"><i class="fa fa-ellipsis-v"></i></a>
                                            <div class="dropdown-menu dropdown-menu-right " x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-91px, 27px, 0px);">
                                                <a class="dropdown-item" :href="route('type.show', [row.id])"><i class="fa fa-pencil m-r-5"></i> View</a>
...
import { Datetime } from 'vue-datetime';
 ...
 export default {
    ...
    methods: {
        ...
        resetData: function () {
            this.resetQuery()
            this.$inertia.get('/type', this.query)
        },
        getData: function () {
            this.$inertia.get('/type', this.query)
        },
        getDateData: function () {
            this.$inertia.get('/type', this.dateQuery)
        },
        deleteRow: function (data) {
            if (!confirm('Are you sure want to remove?')) return;
            data._method = 'DELETE';
            this.$inertia.post('/type/' + data.id, data)
        }
    }
}
</script>
