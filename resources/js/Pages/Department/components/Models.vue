             ...
             <div class="user-serchbox d-flex">
                 ...
                 <div class="searchbox ml-auto">
                     <nav class="navbar user-nav d-flex nav-mr-top">
                         ...
                         <a :href="route(link)" :active="route().current(link)" class="btn btn-primary btn-sm cls rg-top-btn"><i class="fa fa-plus"></i> Department</a>
                     </nav>
                         ...
                         <table style="margin-top: 15px;" class="table table-border table-striped custom-table mb-0">
                             ...
                             <tr v-for="(row,index) in data['data'].data" :key="row.id">
                                 ...
                                 <td v-if="$page.auth.user.user_type == 'staff'" class="text-right">
                                     <div class="dropdown dropdown-action ">
                                         ...
                                         <div class="dropdown-menu dropdown-menu-right " x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-91px, 27px, 0px);">
                                             <a :class="$page.auth.user.id != row.created_by?'disabled':'' " class="dropdown-item" :href="route('departments.show', [row.id])"><i class="fa fa-pencil m-r-5"></i> View</a>
                                 ...
                                 <td v-else class="text-right">
                                    <div class="dropdown dropdown-action ">
                                        <a href="#" class="action-icon dropdown-toggle" style="position:relative" data-toggle="dropdown" aria-expanded="true"><i class="fa fa-ellipsis-v"></i></a>
                                        <div class="dropdown-menu dropdown-menu-right action-btn" x-placement="bottom-end" >
                                            <a class="dropdown-item" :href="route('departments.show', [row.id])"><i class="fa fa-pencil m-r-5"></i> View</a>
                                            <a class="dropdown-item" href="#" data-toggle="modal" data-target="#delete_patient"  @click="deleteRow(row)"><i class="fa fa-trash-o m-r-5"></i> Delete</a>
                                        </div>
                                    </div>
								</td>
                             </tr>
...
import moment from 'moment'
 ...
 export default {
     ...
     data() {
         return {
             filter_types: [
                 ...
                 {'key_value': 'name', 'key_name': 'Department'},
             ],
             query: {
                 search_type: '',
                 search_text: ''
    ...
    methods: {
        ...
        resetData: function () {
            this.resetQuery()
            this.$inertia.get('/departments', this.query)
        },
        getData: function () {
            this.$inertia.get('/departments', this.query)
        },
        getDateData: function () {
            this.$inertia.get('/departments', this.dateQuery)
        },
        ...
        getHumanDate : function (date) {
                return moment(date, 'YYYY-MM-DD').format('DD-MM-YYY');
            }
    }
}
</script>
