...
<!--                        <a :href="route(link)" :active="route().current(link)" class="btn btn-primary btn-sm cls rg-top-btn"><i class="fa fa-plus"></i> Submissions</a>-->
                    </nav>
...
<!--                        temporary Remove datatable-->
                        <table  style="margin-top: 15px;" class="table table-border table-striped custom-table mb-0">
                            ...
                            <tr v-for="(row, index) in data['data'].data" :key="row.id">
                                ...
                                <td>{{row.form_responses.length}}</td>
                                ...
                                <td class="text-right">
                                        <div class="dropdown dropdown-action ">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right " x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-91px, 27px, 0px);">
                                                <a class="dropdown-item" :href="route('form.response.show', [row.id])"><i class="fa fa-pencil m-r-5"></i> View</a>
...
export default {
    ...
    methods: {
        ...
        resetData: function () {
            this.resetQuery()
            this.$inertia.get('/forms/data', this.query)
        },
        getData: function () {
            this.$inertia.get('/forms/data', this.query)
        },
        getDateData: function () {
            this.$inertia.get('/forms/data', this.dateQuery)
        },
