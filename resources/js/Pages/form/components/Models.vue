            ...
            <div class="user-serchbox d-flex">
                ...
                <div class="searchbox ml-auto">
                    <nav class="navbar user-nav d-flex nav-mr-top">
                        ...
                        <a v-if ="$page.auth.user.user_type != 'member'" :href="route(link)" :active="route().current(link)" class="btn btn-primary btn-sm cls rg-top-btn"><i class="fa fa-plus"></i> Form</a>
                    </nav>
...
<!--                                <label class="date_export bold"  >Export: </label>-->
...
<!--                        Temporary remove datatable-->
                        <table  style="margin-top: 15px;" class="table table-border table-striped custom-table mb-0">
                            ...
                            <tr v-for="(row,index) in data['data'].data" :key="row.id">
                                ...
                                <td v-if="$page.auth.user.user_type == 'staff' || $page.auth.user.user_type == 'member'" class="text-right">
                                    <div class="dropdown dropdown-action ">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right " x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-91px, 27px, 0px);">
                                            ...
                                            <a class="dropdown-item" :href="route('forms.response', [row.slug])"  target="_blank"><i class="fa fa-pencil m-r-5"></i> View</a>
                                            <a :class="$page.auth.user.id != row.created_by?'disabled':'' " class="dropdown-item" :href="route('forms.edit', [row.id])"><i class="fa fa-edit"></i> Edit</a>
                                ...
                                <td v-if="$page.auth.user.user_type == 'admin' || $page.auth.user.user_type == 'owner'" class="text-right">
                                        <div class="dropdown dropdown-action ">
                                            <a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="true"><i class="fa fa-ellipsis-v"></i></a>
                                            <div class="dropdown-menu dropdown-menu-right " x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-91px, 27px, 0px);">
                                                <button @click="setActiveResponse(row.id)" class="dropdown-item"  data-toggle="modal" data-target="#updateStatus"><i class="fa fa-plus m-r-5"></i> Create Task</button>
                                                <a class="dropdown-item" :href="route('forms.response', [row.slug])" target="_blank"><i class="fa fa-pencil m-r-5"></i> View</a>
                                                <a class="dropdown-item" :href="route('forms.edit', [row.id])"><i class="fa fa-edit"></i> Edit</a>
...
import moment from "moment";
 ...
 export default {
    ...
    props: ['data','title','flash','errors','link','label'],
     data() {
         return {
             base_url:'https://workfloweasy.ca/form/response/',
             filter_types: [
    ...
    methods: {
        ...
        save: function (params) {
            ...
            params['task_reference_type'] = 'Form';
        ...
        resetData: function () {
            this.resetQuery()
            this.$inertia.get('/forms', this.query)
        },
        getData: function () {
            this.$inertia.get('/forms', this.query)
        },
        getDateData: function () {
            this.$inertia.get('/forms', this.dateQuery)
        },
        deleteRow: function (data) {
            if (!confirm('Are you sure want to remove?')) return;
            data._method = 'DELETE';
            this.$inertia.post('/forms/' + data.id, data)
        }
    }
}
</script>
