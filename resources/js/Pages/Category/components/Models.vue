            ...
            <div class="user-serchbox d-flex">
                ...
                <div class="searchbox ml-auto">
                    <nav class="navbar user-nav d-flex nav-mr-top">
                        ...
                        <a :href="route(link)" :active="route().current(link)" class="btn btn-primary btn-sm cls rg-top-btn"><i class="fa fa-plus"></i> Category</a>
                    </nav>
                                ...
                                <td v-if="$page.auth.user.user_type == 'staff'" class="text-right">
                                    <div class="dropdown dropdown-action ">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right " x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-91px, 27px, 0px);">
                                            <a :class="$page.auth.user.id != row.created_by?'disabled':'' " class="dropdown-item" :href="route('categories.show', [row.id])"><i class="fa fa-pencil m-r-5"></i> View</a>
...
import { Datetime } from 'vue-datetime';
 ...
 export default {
    ...
    methods: {
        ...
        resetData: function () {
            this.resetQuery()
            this.$inertia.get('/categories', this.query)
        },
        getData: function () {
            this.$inertia.get('/categories', this.query)
        },
        getDateData: function () {
            this.$inertia.get('/categories', this.dateQuery)
        },
