             ...
             <div class="user-serchbox d-flex">
                 ...
                 <div class="searchbox ml-auto">
                     <nav class="navbar user-nav d-flex nav-mr-top">
                         ...
                         <a :href="route(link)" :active="route().current(link)" class="btn btn-primary btn-sm cls rg-top-btn"><i class="fa fa-plus"></i> {{title}}</a>
                     ...
                     <div class="table-responsive">
                         ...
                         <table class="table table-border table-striped custom-table datatable mb-0">
                             ...
                             <tr v-for="(row,index) in data['data'].data" :key="row.id">
                                 ...
                                 <td>{{ row.subscription ? row.subscription.name : null}}</td>
                                 <td>
                                     <button v-if="row.status == 1" class="custom-badge btn grn">Active</button>
                                     <button v-else-if="row.status == 2" class="custom-badge btn ylw">Maintenance</button>
                                     <button v-else class="custom-badge btn rd">Suspended</button>
                                 </td>
                                 <td class="text-right">
                                        <div class="dropdown dropdown-action ">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right " x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-91px, 27px, 0px);">
                                                <a class="dropdown-item" :href="route('organization-users.show', [row.id])"><i class="fa fa-pencil m-r-5"></i> View</a>
...
import JetResponsiveNavLink from '@/Jetstream/ResponsiveNavLink'
 ...
 export default {
     ...
     data() {
         return {
             filter_types: [
                 ...
                 {'key_value': 'code', 'key_name': 'Code'},
                 {'key_value': 'email', 'key_name': 'Email'},
                 {'key_value': 'mobile', 'key_name': 'Phone'},
                 {'key_value': 'website', 'key_name': 'Website'},
             ],
             query: {
                 search_type: '',
                 search_text: ''
    ...
    methods: {
        ...
        resetData: function () {
            this.resetQuery()
            this.$inertia.get('/organization-users', this.query)
        },
        getData: function () {
            this.$inertia.get('/organization-users', this.query)
        },
        getDateData: function () {
            this.$inertia.get('/organization-users', this.dateQuery)
        },
        deleteRow: function (data) {
            if (!confirm('Are you sure want to remove?')) return;
            data._method = 'DELETE';
            this.$inertia.post('/organization-users/' + data.id, data)
        }
    }
}
</script>
