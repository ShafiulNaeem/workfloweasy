            ...
            <div class="user-serchbox d-flex">
                ...
                <div class="searchbox ml-auto">
                    <nav class="navbar user-nav d-flex nav-mr-top">
                        ...
                        <a :href="route(link)" :active="route().current(link)" class="btn btn-primary btn-sm cls rg-top-btn"><i class="fa fa-plus"></i> Automation</a>
                    </nav>
...
<!--                        Temporary remove datatable-->
                        <table  style="margin-top: 15px;" class="table table-border table-striped custom-table mb-0">
                            ...
                            <tr v-for="(row,index) in data['data'].data" :key="row.id">
                                ...
                                <td> <span class="badge badge-info" v-for="d in row.users">{{d.name}} </span> </td>
                                <td>{{ row.due_date}}.00 Hours</td>
                                 ...
                                 <td v-if="$page.auth.user.user_type == 'staff'" class="text-right">
                                        <div class="dropdown dropdown-action ">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right " x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-91px, 27px, 0px);">
                                                <a class="dropdown-item" :class="$page.auth.user.id != row.created_by?'disabled':'' " :href="route('formAutomation.show', [row.id])"><i class="fa fa-pencil m-r-5"></i> View</a>
                                 ...
                                 <td v-else class="text-right">
                                        <div class="dropdown dropdown-action ">
                                            <a href="#" class="action-icon dropdown-toggle" data-toggle="dropdown" aria-expanded="true"><i class="fa fa-ellipsis-v"></i></a>
                                            <div class="dropdown-menu dropdown-menu-right " x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(-91px, 27px, 0px);">
                                                <a class="dropdown-item" :href="route('formAutomation.show', [row.id])"><i class="fa fa-pencil m-r-5"></i> View</a>
...
import { Datetime } from 'vue-datetime';
 ...
 export default {
    ...
    methods: {
        ...
        resetData: function () {
            this.resetQuery()
            this.$inertia.get('/formAutomation', this.query)
        },
        getData: function () {
            this.$inertia.get('/formAutomation', this.query)
        },
        getDateData: function () {
            this.$inertia.get('/formAutomation', this.dateQuery)
        },
        deleteRow: function (data) {
            if (!confirm('Are you sure want to remove?')) return;
            data._method = 'DELETE';
            this.$inertia.post('/formAutomation/' + data.id, data)
        }
    }
}
</script>
